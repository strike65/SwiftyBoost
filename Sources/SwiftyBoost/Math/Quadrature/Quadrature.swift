...
public enum Quadrature {
    ...
    ///   let gk = try Quadrature.Integrator<Double>(rule: .gaussKronrod(points: 61))
...
private extension Quadrature.Rule {
    ...
    static func checkedCount(_ value: Int, label: String) throws -> Int32 {
        guard value > 0 else {
            throw Quadrature.Error.invalidConfiguration("\(label) requires a positive point count (received \(value)).")
        }
        guard value <= Int(Int32.max) else {
            throw Quadrature.Error.invalidConfiguration("\(label) point count exceeds Int32.max (received \(value)).")
        }
        return Int32(value)
    }
    ...
    static func normalizedAdaptiveParameters<Scalar: BinaryFloatingPoint>(
        ...
        guard maxRefinements > 0 else {
            throw Quadrature.Error.invalidConfiguration("\(label) maxRefinements must be positive (received \(maxRef ?? 0)).")
        }
        guard maxRefinements <= Int(Int32.max) else {
            throw Quadrature.Error.invalidConfiguration("\(label) maxRefinements exceeds Int32.max (received \(maxRefinements)).")
        }
        let tol = tolerance ?? Scalar(1e-9)
        guard tol > 0 else {
            throw Quadrature.Error.invalidConfiguration("\(label) tolerance must be positive (received \(Double(tolerance ?? 0))).")
        }
...
private enum QuadratureBridge {
    ...
    static func createHandle<Scalar: Real & BinaryFloatingPoint & Sendable>(
        ...
        if Scalar.self == Double.self {
            ...
            #endif
            throw Quadrature.Error.invalidConfiguration("Unsupported scalar type: \(Scalar.self)")
        }
    }
    ...
    static func integrate<Scalar: Real & BinaryFloatingPoint & Sendable>(
        ...
        over interval: Quadrature.Interval,
        integrand: @escaping @Sendable (Scalar) -> Scalar,
        scalar _: Scalar.Type
...
private enum QuadratureBridgeDouble {
    static func createHandle(for rule: Quadrature.Rule) throws -> QuadratureHandle {
        switch rule {
        case let .gaussLegendre(points):
            let n = try Quadrature.Rule.checkedCount(points, label: "Gauss-Legendre")
            guard let handle = quad_gauss_create_d(n) else {
                throw Quadrature.Error.invalidConfiguration("Gauss-Legendre does not support \(points) points.")
            }
            return handle
        case let .gaussKronrod(points):
            let n = try Quadrature.Rule.checkedCount(points, label: "Gauss-Kronrod")
            guard let handle = quad_gauss_kronrod_create_d(n) else {
                throw Quadrature.Error.invalidConfiguration("Gauss-Kronrod does not support \(points) points.")
            }
            return handle
        case let .tanhSinh(maxRefinements, tolerance):
            if maxRefinements == nil && tolerance == nil {
                guard let handle = quad_tanh_sinh_create_d() else {
                    throw Quadrature.Error.backendUnavailable("Tanh-Sinh integrator is unavailable.")
                }
                return handle
            }
            let (maxRef, tol) = try Quadrature.Rule
            ...
            guard let handle = quad_tanh_sinh_create_with_params_d(maxRef, tol) else {
                throw Quadrature.Error.invalidConfiguration("Tanh-Sinh integrator rejected maxRef=\(maxRefinements ?? 0), tolerance=\(tolerance ?? 0).")
            }
            return handle
        case let .sinhSinh(maxRefinements, tolerance):
            if maxRefinements == nil && tolerance == nil {
                guard let handle = quad_sinh_sinh_create_d() else {
                    throw Quadrature.Error.backendUnavailable("Sinh-Sinh integrator is unavailable.")
                }
                return handle
            }
            let (maxRef, tol) = try Quadrature.Rule
            ...
            guard let handle = quad_sinh_sinh_create_with_params_d(maxRef, tol) else {
                throw Quadrature.Error.invalidConfiguration("Sinh-Sinh integrator rejected maxRef=\(maxRefinements ?? 0), tolerance=\(tolerance ?? 0).")
            }
            return handle
        case let .expSinh(maxRefinements, tolerance):
            if maxRefinements == nil && tolerance == nil {
                guard let handle = quad_exp_sinh_create_d() else {
                    throw Quadrature.Error.backendUnavailable("Exp-Sinh integrator is unavailable.")
                }
                return handle
            }
            let (maxRef, tol) = try Quadrature.Rule
            ...
            guard let handle = quad_exp_sinh_create_with_params_d(maxRef, tol) else {
                throw Quadrature.Error.invalidConfiguration("Exp-Sinh integrator rejected maxRef=\(maxRefinements ?? 0), tolerance=\(tolerance ?? 0).")
            }
            return handle
        }
    }
        ...
        let raw: QuadratureResultD = try withCallbackDouble(integrand) { callback, context in
            ...
            case let .finite(lower, upper):
                guard lower.isFinite, upper.isFinite, lower < upper else {
                    throw Quadrature.Error.invalidInterval(lower: lower, upper: upper)
                }
